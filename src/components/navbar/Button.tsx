...
export default function Button({ isLink }: { isLink: boolean }) {
  const handleClick = () => {
    isLink ? router.push('/') : setIsOpen(!isOpen);
  ...
  return (
    <button
      onClick={() => {
        isLink ? router.push('/') : setIsOpen(isOpen ? false : true);
      }}
